...
function App() {
  ...
  const fetchInfo = (fetchPlay) => {
    let filename = fetchPlay.filename;
    if (filename) {
      console.log("fetching play: " + fetchPlay.id);
      setFileExists(true);
      filename = filename.substring(0, filename.lastIndexOf(".")); //removes the .json extension in the filename
      // const url ="https://www.randyconnolly.com//funwebdev/3rd/api/shakespeare/play.php?name=" + filename;
      // console.log("/api/play/" + fetchPlay.id);
      // console.log("look above idiot");
      const url = "/api/play/" + fetchPlay.id;
